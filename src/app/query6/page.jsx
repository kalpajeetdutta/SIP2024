...
const ScreenTimeChartsix = () => {
  ...
  const fetchData = () => {
    fetch('http://localhost:8080/api/data/get', {
      ...
      .then(data => {
        ...
        const jointFamilyData = data.filter(child => child.familyType === 'joint' && child.gender === 'female');
        const nuclearFamilyData = data.filter(child => child.familyType === 'nuclear' && child.gender === 'female');
  ...
  const processScreenTimeData = (data) => {
    ...
    const totalScreenTime = Object.values(programScreenTime).reduce((acc, cur) => acc + cur, 0);
    const screenTimePercentages = {};
    programs.forEach(program => {
      screenTimePercentages[program] = (programScreenTime[program] / totalScreenTime) * 100;
    });
  ...
  if (!jointFamily || !nuclearFamily || !totalData) return (
  ...
  return (
    <div className="w-full h-full overflow-x-hidden p-5 md:px-10">
      ...
      Query: Graphical representation of screen time percentage(%) of different programs watched by female children in joint and nuclear family
      </h1>
      ...
      <div className="mt-5 w-full md:flex md:flex-col md:items-center">
        ...
        <table>
          ...
          <tr className='bg-gray-200 text-left text-gray-700 font-medium'>
                ...
                <th className='px-4 py-2 text-center'>Joint Family Percentage (%)</th>
                <th className='px-4 py-2 text-center'>Nuclear Family Percentage (%)</th>
                <th className='px-4 py-2 text-center'>Total Percentage (%)</th>
              </tr>
          </thead>
          <tbody>
          {Object.keys(jointFamily).map((program) => (
                <tr key={program} className='border-b border-gray-200 hover:bg-gray-100'>
                  <td className='px-4 py-2 text-center'>{program.charAt(0).toUpperCase() + program.slice(1)}</td>
                  <td className='px-4 py-2 text-center'>{jointFamily[program].toFixed(2)}%</td>
                  <td className='px-4 py-2 text-center'>{nuclearFamily[program].toFixed(2)}%</td>
                  <td className='px-4 py-2 text-center'>{totalData[program].toFixed(2)}%</td>
                </tr>
              ))}
          </tbody>
        </table>
